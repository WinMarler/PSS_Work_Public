...
def generate_html_table(seq_number):
    ...
    material_field = "Material" if len(materials) == 1 else "Material Options"
    base_fields.append(material_field)
    ...
    # 处理附加信息，放入括号并用空格连接
    main_method = installation_method  # 直接使用 get_installation_method 的返回值作为主方法
    extra_info = []
    if need_remove and need_remove.strip() == "是":
        extra_info.append("For best results, bumper removal recommended")
    if notes and notes.strip():
        extra_info.append(notes.strip())
    if extra_info:
        installation_method = f"{main_method} ({' '.join(extra_info)})"
    ...
    table_data.append(f"{material_field} :")
    if len(materials) == 0:
    ...
    # Pattern Options（纯文本，仅在非全非碳纤维材质时生成）
    if not is_non_carbon_only:
        table_data.append("Pattern Options :")
        if len(patterns) == 0:
    ...
    if not is_non_carbon_only:
        if len(finishes) == 1 and "gloss" in finishes[0].lower():
            ...
            if has_gloss or len(finishes) == 0:
            ...
            table_data.append(f"Finish Options : {' / '.join(finish_options)}")
        else:
            if has_frp:
    ...
    if "bumper" in product_name.lower():
        bumper_note = "Please note before purchasing\n-For bumpers made with FRP or Carbon material, please expect some modifications and adjustments for the best results, sanding, drilling and adding body filler is to be expected."
        additional_info = f"{additional_info}\n{bumper_note}" if additional_info != "-" else bumper_note
    ...
    # Pattern Options（HTML，仅在非全非碳纤维材质时生成）
    pattern_html = ""
    if not is_non_carbon_only:
        pattern_map = {
            "woven 3k": "斜纹3K",
    ...
    for field in fields:
        ...
        if field == "Vehicle Model & Chassis":
            ...
            value = model_chassis
        elif field == material_field:
            value = material_html
        elif field == "Pattern Options":
            value = pattern_html
            ...
            for line in table_data:
                if line.startswith(f"{field} :"):
                    ...
                    break
        # 特殊处理 Vehicle Body Type 和 Additional Information，添加 <br> 分隔
        if field in ["Vehicle Body Type", "Additional Information"] and value:
            value = value.replace('\n', '<br>')
        if field == "Brand":
